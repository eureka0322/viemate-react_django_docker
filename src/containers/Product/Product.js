...
import {} from './Product.scss';
import { Product as ProductComponent, ErrorPage } from 'components';
...
export default class Product extends Component {
  ...
  changeLocation(props) {
    ...
    //   this.props.replace(`/apartments/${location}`);
    // }
  }

  render() {
    const {product} = this.props;
    if ((product && product.post_type === 'wanted')) {
      return (
        <div>
          <ErrorPage />
        </div>
    ...
    return (
      <div>
        <Helmet
          {...config.app.head}
        />
        <div className="container--default-width container">
          {(!this.props.product || !this.props.product.id) && <DisableComponent.Footer />}
          {!!this.props.product && !!this.props.product.id && <ProductComponent />}
        </div>
      </div>
    );
  }
